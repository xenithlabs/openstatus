...
export const page = sqliteTable(
  "page",
  {
    ...
    passwordProtected: integer("password_protected", {
      mode: "boolean",
    }).default(false),
    accessType: text("access_type", { enum: pageAccessTypes }).default(
      "public",
    ),
  ...
  (t) => [
    ...
    index("page_lower_custom_domain_idx").on(sql`LOWER(${t.customDomain})`),
